...
def showdatas(datingDataMat, datingLabels):
    ...
    numberOfLabels = len(datingLabels)  # 获得标签个数
    LabelsColors = []
...
def datingClassTest():
    ...
    # 打开并处理数据
    datingDataMat, datingLabels = File_To_Matrix(filename)
    # 数据可视化
    # showdatas(datingDataMat, datingLabels)

    # 数据归一化
    normDataSet, ranges, minVals = autoNum(datingDataMat)
    # 前10%个数据作为测试集,后90%个数据作为训练集
    ...
    for i in range(numTestVecs):
        ...
        classifierResult = classify(normDataSet[i, :], normDataSet[numTestVecs:m, :], datingLabels[numTestVecs:m], 4)
